...
export const getUserById = async (userId: string) =>
  fetch(`${API_URL}/user/id?user_id=${userId}`).then((res) => res.json());
...
export const getFriendsByUserId = async (userId: string) =>
  fetch(`${API_URL}/friends?user_id=${userId}`, {
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify([]),
...
export const getSuggestFriendByUserId = async (userId: string) =>
  fetch(`${API_URL}/suggest-friend?user_id=${userId}`).then((res) =>
    res.json()
  );

export const sendRelationship = async (body: any) =>
  fetch(`${API_URL}/relationship`, {
    headers: {
      "Content-Type": "application/json",
    },
...
export const checkRelationship = async (user1: string, user2: string) =>
  fetch(`${API_URL}/relationship?user1=${user1}&user2=${user2}`, {
    headers: {
      "Content-Type": "application/json",
    },
  }).then((res) => res.json());
...
export const getFriendUser = async (userId: string, status: any) =>
  fetch(`${API_URL}/users?user_id=${userId}&status=${status}`, {
    headers: {
      "Content-Type": "application/json",
    },
  }).then((res) => res.json());
